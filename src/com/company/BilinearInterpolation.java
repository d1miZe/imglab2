...
public class BilinearInterpolation {
    ...
    public File compress(double koef) {
        ...
        try {
            ...
            String coefString;
            if (koef < 10) {
                coefString = String.valueOf(koef).substring(0, 3).replace('.', '_');
            }else {
                coefString = String.valueOf(koef).substring(0, 4).replace('.', '_');
            }
            String pathToNewFile = Main.PATH_TO_FILE.substring(0, 18) + coefString + ".bmp";
